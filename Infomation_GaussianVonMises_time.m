...
freq=1;
    ...
    for index= 8:length(lfp)
        ...
        for i=1:ncells
            ...
            [pthetan_t pthetan pt]=ConditionalVonMisesProbability(matrix(:,:,i),pmatrix1,pmatrix2);
            for aspect=1:6
                   [I hn hnt ht]=BehaviouralPairInformationSquareProtocol(pthetan,pthetan_t,pt,1,pos,freq,aspect-1)
                   recording_data{index}{i}.I_np_t(aspect)=I;
                   recording_data{index}{i}.h_phasen_t(aspect)=hnt;
                   recording_data{index}{i}.h_phasen(aspect)=hn;
                   recording_data{index}{i}.h_var(aspect)=ht;
            end
        end
        end
    end
